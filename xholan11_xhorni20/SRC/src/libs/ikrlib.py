...
def framing(a, window, shift=1):
    shape = (int((a.shape[0] - window) / shift + 1), int(window))
...
def wav16khz2mfcc(dir_name):
    ...
    for f in glob(dir_name + '/*.wav'):
        ...
        # now remove first 1.5 second because of noise
        number_of_samples_trimmed = int(16000 * 1.5)
        s = s[number_of_samples_trimmed:]
        assert(rate == 16000)
